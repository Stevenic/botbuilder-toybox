...
import { TurnContext, Activity, CardAction, MessageFactory } from 'botbuilder-core'
import { DialogContext, Dialog, DialogTurnResult, findChoices, Choice } from 'botbuilder-dialogs'
 ...
 */
export type ListPager= (list: ListControlContext) => Promise<ListPagerResult>
 ...
 */
export class ListControl extends Dialog<ListControlOptions> {
     ...
     */
    constructor(dialogId: string, private pager: ListPager, actions?: (string|CardAction)[]) { 
        super(dialogId);
        this.actions = actions || [{ type: 'imBack', title: 'Show More', value: 'more' }];
    }

    /** @private */
    public async beginDialog(dc: DialogContext, options?: ListControlOptions): Promise<DialogTurnResult> {
        dc.activeDialog.state = Object.assign({}, options);
        return await this.showMore(dc, true);
    }
    ...
    public async continueDialog(dc: DialogContext): Promise<DialogTurnResult> {
        ...
        if (action === 'more') {
            return await this.showMore(dc, false);
        } else {
            const state = dc.activeDialog.state as ListControlOptions;
            return await dc.endDialog({ noResults: false, action: action, continueToken: state.continueToken });
        }
    }

    private async showMore(dc: DialogContext, noResults: boolean): Promise<DialogTurnResult> {
        const state = dc.activeDialog.state as ListControlOptions;

        const page = await this.pager({ 
            context: dc.context, 
            filter: state.filter, 
        ...
        if (page.result) {
            if (page.continueToken !== undefined) {
                ...
                return await dc.endDialog({ noResults: noResults });
            }
        } else {
            // Just end the dialog
            return await dc.endDialog({ noResults: noResults });
