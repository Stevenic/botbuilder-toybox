 ...
 */
export type VersionChangedHandler = (context: TurnContext, version: number, next: () => Promise<void>) => Promise<void>;
 ...
 * ```
 */
export class CheckVersionMiddleware implements Middleware {
    /**
     ...
     */
    constructor(private versionProperty: StatePropertyAccessor<number>, private version: number, private handler: VersionChangedHandler) { }

    /** @private */
    public async onTurn(context: TurnContext, next: () => Promise<void>): Promise<void> {
        // Check for version change
        let version = await this.versionProperty.get(context, this.version) as number;
        if (version !== this.version) {
            await Promise.resolve(this.handler(context, version, async () => {
                await this.versionProperty.set(context, this.version);
                await next();
            }));
        } else {
            await next();
