...
export interface HttpAdapterSettings {
    /**
     * (Optional) `channelId` to always assign incoming activities. 
     * 
     * ## Remarks
     * By default the adapter will assign a channelId of "http" but only if the channelId is missing.
     * Setting this option ensures that all incoming activities will have the same channelId.
     */
    channelId?: string;
...
export class HttpAdapter extends BotAdapter {
     ...
     */
    public processActivity(req: IncomingRequest, res: IncomingResponse, logic: (context: TurnContext) => Promise<any>): Promise<void> {
    ...
    public async sendActivities(context: TurnContext, activities: Partial<Activity>[]): Promise<ResourceResponse[]> {
        ...
        for (let i = 0; i < activities.length; i++) {
            ...
            if (a.type === 'invokeResponse') {
                ...
                responses.push(r || {} as ResourceResponse);
            } else {
                // Add activity to cached response array
                let activities = context.turnState.get(RESPONSE_ACTIVITIES_KEY) as (Partial<Activity>[]|undefined);
                if (!activities) {
                    activities = [];
